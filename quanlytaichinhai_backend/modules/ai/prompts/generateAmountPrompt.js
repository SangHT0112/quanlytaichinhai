...
export const generateAmountPrompt = async ({ user_input, historyText, now, user_id }) => {
  ...
  return `
    ...
    Yêu cầu:
    - Nếu input là số tiền (ví dụ: "35k", "2tr", "35k nhá") và lịch sử có câu input người dùng trước đó liên quan đến giao dịch ăn uống (ví dụ: "Nay ăn phở quán bà 6"), tạo một giao dịch mới.
    - Sử dụng hoạt động chính từ câu input người dùng gần nhất (bỏ chi tiết như "quán bà 6") làm group_name và description.
    - Trả về JSON hợp lệ:
      {
        "group_name": "Tóm tắt hoạt động (ví dụ: Ăn phở)",
        "transaction_date": "${now}",
        "user_id": ${user_id},
        "total_amount": Số tiền (chuyển đổi k thành 000, tr thành 000000),
        "transactions": [
          {
            "type": "expense",
            "amount": Số tiền,
